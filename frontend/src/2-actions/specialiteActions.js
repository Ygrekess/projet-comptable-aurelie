...
const addSpecialite = (poleId, specialite) => async (dispatch) => {
	...
	try {
		const { data } = await axios.post('/api/specialites/', {specialite: specialite, poleId: poleId})
...
const updateSpecialite = (specialiteId, specialite) => async (dispatch) => {
	...
	try {
		const { data } = await axios.put(`/api/specialites/${specialiteId}`, specialite)
...
const resetSpecialite = () => (dispatch) => {
	dispatch({ type: 'SPECIALITE_UPDATE_RESET' })
}

export { addSpecialite, getAllSpecialites, getOneSpecialite, updateSpecialite, resetSpecialite }